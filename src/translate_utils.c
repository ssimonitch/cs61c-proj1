...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    // parse str and put result into output
    // also set pEnd to the first character after the parsed number
    char* pEnd;
    *output = strtol(str, &pEnd, 0);

    // if we found a character after the parsed number,
    // the string is invalid
    if (*pEnd)
      return -1;
    // check bounds
    else if (*output < lower_bound || *output > upper_bound)
      return -1;
    else
      return 0;
}
